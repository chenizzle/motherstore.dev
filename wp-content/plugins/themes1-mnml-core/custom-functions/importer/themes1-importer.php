 ...
 */
 ...
 // Don't duplicate me!
 if ( !class_exists( 'Themes1_Theme_Importer' ) ) {
	...
	class Themes1_Theme_Importer {
		 ...
		 * @var object
		 */
		public $theme_options_framework = 'the1_themes'; //supports radium framework and option tree
	    ...
	    public function __construct() {
			...
			add_filter( 'add_post_metadata', array( $this, 'the1_check_previous_meta' ), 10, 5 );

      		add_action( 'radium_import_end', array( $this, 'the1_after_wp_importer' ) );
         ...
         * @return
         */
        public function the1_check_previous_meta( $continue, $post_id, $meta_key, $meta_value, $unique ) {
    	 ...
    	 * @since 0.0.2
    	 */
    	public function the1_after_wp_importer() {
	     ...
	     * @return null
	     */
	    public function the1_process_imports( $content = true,  $widgets = true, $demofolder ) {
			
			$xmlpath = get_template_directory() . '/core/importer/demo-files/'.esc_attr($demofolder).'/content.xml';
			if ( $content && !empty( $xmlpath ) && is_file( $xmlpath ) ) {
				$this->the1_set_demo_data( $xmlpath );
			} else {
				echo esc_attr($demofolder) . "/content.xml.gzip not found!<br />";
			}
			
			$jsonpath = get_template_directory() . '/core/importer/demo-files/' . esc_attr($demofolder ) . '/widgets.json';
			if ( $widgets && !empty( $jsonpath ) && is_file( $jsonpath ) ) {
				$this->process_widget_import_file( $jsonpath );
			} else {
				echo esc_attr($demofolder) . "/widgets.json not found!<br />";
			}
	    ...
	    /**
	     * the1_add_widget_to_sidebar Import sidebars
	     ...
	     * @return null
	     */
	    public function the1_add_widget_to_sidebar($sidebar_slug, $widget_slug, $count_mod, $widget_settings = array()){
	    ...
	    public function the1_set_demo_data( $file ) {
	    ...
	    public function the1_set_demo_menus() {}
	
	    public function the1_set_demo_theme_options( $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				if( $this->theme_options_framework == 'the1_themes') {
				...
				if ( !empty( $data ) || is_array( $data ) ) {
	      		...
	      		wp_die(
      				__( 'Theme options Import file could not be found. Please try again.', 'themes1-mnml-core' ),
      				'',
	    ...
	    function process_widget_import_file( $file ) {
	    	...
	    	if ( ! file_exists( $file ) ) {
	    		wp_die(
	    			__( 'Widget Import file could not be found. Please try again.', 'themes1-mnml-core' ),
	    			'',
	    			array( 'back_link' => true )
	    		);
	    	}
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
	    			...
	    			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'themes1-mnml-core' );
	    		}
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
	    				...
	    				$widget_message = __( 'Site does not support widget', 'themes1-mnml-core' ); // explain why widget not imported
	    			}
	    			...
	    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
	    				...
	    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
	    					...
	    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
	    						...
	    						$widget_message = __( 'Widget already exists', 'themes1-mnml-core' ); // explain why widget not imported
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = __( 'Imported', 'themes1-mnml-core' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = __( 'Imported to Inactive', 'themes1-mnml-core' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'themes1-mnml-core' ); // show "No Title" if widget instance is untitled
